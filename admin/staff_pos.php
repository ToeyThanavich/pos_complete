...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['place_order'])) {
  ...
  if ($table_id && !empty($items)) {
        // 1. เช็กว่าโต๊ะนี้มีบิลที่ยังไม่จ่าย (สถานะยังไม่เป็น completed/cancelled) ค้างอยู่หรือไม่
        $check_active = $conn->query("SELECT order_id FROM orders WHERE table_id = $table_id AND status NOT IN ('completed', 'cancelled') LIMIT 1");
        
        if ($check_active->num_rows > 0) {
            // กรณีที่ 1: ลูกค้าคนเดิมสั่งเพิ่ม -> ใช้ order_id ของบิลเดิม
            $active_order = $check_active->fetch_assoc();
            $oid = $active_order['order_id'];
        } else {
            // กรณีที่ 2: ลูกค้าใหม่ (บิลเก่าจ่าย/ยกเลิกไปแล้ว) -> สร้างบิลใหม่
        ...
        // 2. นำ order_id ($oid) ที่ได้ ไปบันทึกรายการอาหารใหม่ลง order_items
        foreach ($items as $it) {
            $iid   = (int)$it['id'];
            $qty   = (int)$it['qty'];
...
// POST: ยกเลิกเมนูรายการเดียว (cancel individual item)
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['cancel_item'])) {
    ...
    // If the order is now empty, cancel the whole order
    ...
    header("Location: staff_pos.php"); exit;
}
...
<head>
  ...
  .pay-card-header { padding:12px 16px 10px; border-bottom:1px solid rgba(44,24,16,.06); display:flex; align-items:center; justify-content:space-between; }
  ...
  .pay-items { padding:8px 16px; border-bottom:1px solid rgba(44,24,16,.06); }
  .pay-item-row { display:flex; justify-content:space-between; align-items:flex-start; padding:4px 0; }
...
<body>
  ...
  <div class="main">
    ...
    <script>
    setTimeout(() => document.getElementById('toastSuccess')?.remove(), 4000)
    </script>
    <?php endif; ?>

    <div class="page-content">

      <!-- ─── Active Orders Panel (cancel individual items) ─── -->
        <?php
        $active_orders = $conn->query("
            SELECT o.order_id, o.order_code, o.status, t.table_name
            FROM orders o
            JOIN tables t ON o.table_id=t.table_id
            WHERE o.status NOT IN ('completed','cancelled')
            ORDER BY o.created_at ASC
        ");
        $active_list = $active_orders ? $active_orders->fetch_all(MYSQLI_ASSOC) : [];
        if (!empty($active_list)): ?>
        <div style="margin-bottom:20px;">
          <div class="ord-section-title" style="margin-bottom:12px;">🧾 บิลที่เปิดอยู่ (คลิก × เพื่อยกเลิกรายการ)</div>
          <div style="display:flex;flex-wrap:wrap;gap:10px;">
            <?php foreach ($active_list as $ao):
              $ai_stmt = $conn->prepare("SELECT oi.order_item_id, m.item_name, oi.quantity, oi.price, oi.note FROM order_items oi JOIN menu_items m ON oi.item_id=m.item_id WHERE oi.order_id=?");
              $ai_stmt->bind_param('i',$ao['order_id']); $ai_stmt->execute();
              $ai_items = $ai_stmt->get_result()->fetch_all(MYSQLI_ASSOC);
              if (empty($ai_items)) continue;
              $subtotal = array_sum(array_map(fn($r)=>$r['price']*$r['quantity'], $ai_items));
              $status_labels = ['pending'=>'รอรับ','cooking'=>'กำลังทำ','serving'=>'พร้อมเสิร์ฟ'];
              $status_cls    = ['pending'=>'bs-badge-pending','cooking'=>'bs-badge-cooking','serving'=>'bs-badge-serving'];
            ?>
            <div class="pay-card" style="min-width:260px;max-width:320px;flex:1;">
              <div class="pay-card-header">
                <div>
                  <div class="pay-order-code"><?= htmlspecialchars($ao['order_code']) ?></div>
                  <div class="pay-table">🪑 <?= htmlspecialchars($ao['table_name']) ?></div>
                </div>
                <span class="bs-badge <?= $status_cls[$ao['status']] ?? 'bs-badge-pending' ?>"><?= $status_labels[$ao['status']] ?? $ao['status'] ?></span>
              </div>
              <div class="pay-items">
                <?php foreach ($ai_items as $it): ?>
                <div class="pay-item-row">
                  <span style="display:flex;align-items:flex-start;gap:6px;">
                    <form method="post" style="margin:0;" onsubmit="return confirm('ยกเลิกเมนูนี้?')">
                      ...
                      <button type="submit" style="background:none;border:none;color:#e74c3c;cursor:pointer;font-size:1.1rem;padding:0 2px;line-height:1;font-weight:700;" title="ยกเลิกรายการนี้">×</button>
                    </form>
                  ...
                  <span style="font-size:.78rem;"><?= number_format($it['price']*$it['quantity'],0) ?> ฿</span>
                </div>
                <?php endforeach; ?>
              </div>
              <div style="padding:8px 16px 12px;font-size:.75rem;color:var(--choco-light);">
                รวม: <strong style="color:var(--choco);font-family:'Cormorant Garamond',serif;font-size:1rem;"><?= number_format($subtotal,0) ?> ฿</strong>
              </div>
            </div>
            <?php endforeach; ?>
          </div>
        ...
        <div class="pos-menu">
          <div class="pos-cat-bar" id="catBar">
            ...
            <button class="pos-cat-btn"
              data-cat="<?= $c['category_id'] ?>"><?= htmlspecialchars($c['category_name']) ?></button>
            <?php endforeach; ?>
          </div>
          <div class="pos-menu-grid" id="menuGrid">
            <?php foreach ($cat_list as $c):
              $cid = (int)$c['category_id'];
              $mitems = $conn->query("SELECT * FROM menu_items WHERE active=1 AND category_id=$cid ORDER BY item_name");
              while ($m = $mitems->fetch_assoc()):
            ?>
        ...
        <div class="pos-cart">
          <div class="pos-cart-header">
            ...
            <div class="pos-type-row">
              <button class="pos-type-btn active" data-type="dine_in" onclick="setType('dine_in',this)">🪑
                ทานที่ร้าน</button>
          ...
          <div class="pos-cart-footer">
            ...
            <textarea class="pos-note-input" id="orderNote" rows="2"
              placeholder="หมายเหตุถึงครัว (ไม่บังคับ)…"></textarea>
  ...
  function closeNote() {
    document.getElementById('qnoteOverlay').classList.remove('open');
  }

  function chQty(d) {
    qQty = Math.max(1, qQty + d);
    document.getElementById('qnoteQty').textContent = qQty;
  }
  ...
  function renderCart() {
    ...
    if (!cart.length) {
      el.innerHTML = '<div class="pos-cart-empty">ยังไม่มีรายการ<br><small>กดเมนูด้านซ้ายเพื่อเพิ่ม</small></div>';
      updateTotal();
      return;
    }
  ...
  function updateTotal() {
    ...
    document.getElementById('totalAmt').textContent = '฿ ' + total.toLocaleString('th-TH', {
  ...
  function submitOrder() {
    ...
    if (!cart.length) {
      alert('กรุณาเพิ่มรายการอาหารก่อน');
      return;
    }
